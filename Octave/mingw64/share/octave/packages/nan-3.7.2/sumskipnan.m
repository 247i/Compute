...
%   Copyright (C) 2000-2005,2009,2011,2025 by Alois Schloegl <alois.schloegl@gmail.com>
...
if ~isempty(W) 
	warning('weighted sumskipnan recommends using sumskipnan_mex from nan package (pkg load nan) for better performance');
end;
% below a slower implementation
...
if nargout>1,
	if isempty(W)
		...
		FLAG_NANS_OCCURED = any(count(:)<size(x,DIM));
	        ...
	        count = sum((x==x).*repmat(W,sz),DIM);
		FLAG_NANS_OCCURED = any(count(:)<sum(W));
	end
end;
