## Copyright (C) 2008-2026 Jaroslav Hajek <highegg@gmail.com>
##
...
## if the current user has all the specified types of access, either
...
if (isempty (s))
  ...
  if (nargin >= 2 && ! isempty (tspec))
    ...
    for c = tspec
      ...
      case 'S'
        ft |= S_ISSOCK (s.mode);
      otherwise
        error ("fexist: invalid file type spec: %s", c)
      endswitch
    endfor
    ex &= ft;
  endif
  if (ex && nargin >= 3 && ! isempty (aspec))
    ...
    for c = aspec
      ...
      otherwise
        error ("fexist: invalid access type spec: %s", c)
      endswitch
    endfor
    ex &= at;
  endif
